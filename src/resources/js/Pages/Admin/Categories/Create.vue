...
const submit = () => {
    formData.post('/admin/categories');
};
</script>

<template>
    <Head :title="t('categories.createNew')" />
    <AdminLayout :auth="auth">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ t('categories.createNew') }}</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex justify-end gap-4">
                <Link href="/admin/categories" class="btn btn-secondary">Zrusit</Link>
                <button type="submit" :disabled="formData.processing" class="btn btn-primary">
                    {{ formData.processing ? 'Ukladam...' : 'Vytvorit kategoriu' }}
                </button>
            </div>
        </form>
    </AdminLayout>
